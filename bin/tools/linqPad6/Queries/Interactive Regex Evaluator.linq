...
void Main()
{
	...
	foreach (var control in helpBoxes.Cast<Control>().Append (inputBox).Append (patternBox))
...
object GetResult (RegexOptions checkedOptions) =>
	...
	select new
	{
		...
		Value = new Hyperlink (m.Value,
			h => inputBox.HtmlElement.InvokeScript (false, "eval", $"{inputBox.HtmlElement.ID}.setSelectionRange({m.Index}, {m.Index + m.Length})")),
		m.Success,
		m.Name,
		m.Groups
	};
