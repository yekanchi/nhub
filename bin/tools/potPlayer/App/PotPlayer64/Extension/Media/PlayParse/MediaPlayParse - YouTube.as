...
string GetJsonCode(string data, string code)
{
	int start = data.find(code);
...
//HostOpenConsole();
	...
	if (PlayitemCheck(path))
	{
		...
		WebData = HostUrlGetString(linkWeb, GetUserAgent(), "", "", true);
		...
		if (js_data.empty() && (@QualityList !is null) && !WebData.empty())
		{
			...
			if (jsUrl.empty())
			{
				...
				string JSData = HostUrlGetString(link, GetUserAgent(), "", "", true);
			...
			js_data = HostUrlGetString(jsUrl, GetUserAgent(), "", "", true);
			if (!js_data.empty() && !linkApi.empty())
			{
				if (sts.empty()) sts = HostRegExpParse(js_data, "\"sts\"\\s*:\\s*(\\d+)");
				linkApi = linkApi + "&sts=" + sts;
			}
		}
		...
		for (int z = 0; z < 2; z++)
		{
			...
			else strData = ApiData;
			if (strData.empty()) break;

			string player_response_jsonData, player_chapter_jsonData;
			if (isAPI)
			{
				string decString = HostUrlDecode(strData);
				
				player_response_jsonData = GetJsonCode(decString, MATCH_PLAYER_RESPONSE2);
			}
			else
			{
				player_response_jsonData = GetJsonCode(strData, MATCH_PLAYER_RESPONSE);
				player_response_jsonData.replace("\\/", "/");
				player_response_jsonData.replace("\\\"", "\"");
				player_response_jsonData.replace("\\\\", "\\");
				player_chapter_jsonData = GetJsonCode(strData, MATCH_CHAPTER_RESPONSE);
			...
			if (dashmpd_start <= 0 && (dashmpd_start = strData.find(MATCH_DASHMPD_START)) >= 0)
			{
				...
				dashmpd_len = strData.find(MATCH_END2, dashmpd_start + 10);
				dashmpd_len -= dashmpd_len;
			}

			// hls live streaming
			if (hlsmpd_start <= 0 && (hlsmpd_start = strData.find(MATCH_HLSMPD_START)) >= 0)
			{
				hlsmpd_start += MATCH_HLSMPD_START.size();
				hlsmpd_len = strData.find(MATCH_END2, hlsmpd_start + 10);
			...
			if (player_response_jsonData.empty() && stream_map_len < 0 && hlsmpd_len <= 0)
			{
				if (!isAPI)
				{
					ApiData = HostUrlGetString(linkApi, GetUserAgent(), "", "", true);
					continue;
				}
				break;
			}
			...
			if (@MetaData !is null)
			{
				string dat = HostUrlDecode(strData);
				string title, thumb;
				
				MetaData["webUrl"] = "http://www.youtube.com/watch?v=" + videoId;
				if (isAPI)
				{
					string search = "\"thumbnails\"";
					int first = dat.find(search);
					if (first >= 0)
					{
						search = "\"https";
						first = dat.find(search, first + 10);
						if (first >= 0)
						{
							int next = dat.find("\"", first + 10);
							if (next > first)
							{
								thumb = dat.substr(first, next - first);
								thumb.Trim("\"");
							}
						}
					}
					...
					search = "\"title\":\"";
					first = dat.find(search);
					if (first >= 0)
					{
						first += search.size();
						int next = dat.find("\"", first);
						if (next > first)
						{
							title = dat.substr(first, next - first);
					...
					if (first >= 0)
					{
						...
						if (next > first)
						{
							string view_count = dat.substr(first, next - first);
					...
					if (first >= 0)
					{
						...
						if (next > first)
						{
							string like_count = dat.substr(first, next - first);

							MetaData["likeCount"] = HostUrlDecode(like_count);
						}
					}
					...
					if (first >= 0)
					{
						...
						if (next > first)
						{
							string dislike_count = dat.substr(first, next - first);

							MetaData["dislikeCount"] = HostUrlDecode(dislike_count);
						}
					}
				}
				else
			...
			if (hlsmpd_len > 0)
			{
				string str = strData.substr(hlsmpd_start, hlsmpd_len);					
									...
									if (format.isObject())
									{
										...
										else if (cipher.isString() || signatureCipher.isString())
										{
											...
											if (!u.empty() && !signature.empty() && !js_data.empty())
											{
												...
												u = SignatureDecode(u, signature, param, strData, js_data, JSFuncs, JSFuncArgs);
											}
											item.url = u;
										}
										item.url.replace("\\u0026", "&");
				...
				if (!IsOK)
				{
					...
					for (int i = 0, len = lines.size(); i < len; i++)
					{
						...
						else if (!signature.empty() && !js_data.empty())
						{
							...
							url = SignatureDecode(url, signature, param, strData, js_data, JSFuncs, JSFuncArgs);
						}
						if (itag > 0)
						{
											...
											if (BaseURL.isValid())
											{
												...
												if (!url.empty())
												{
													...
													if (itag > 0)
													{
														...
														if (IsLive) url = url + "?ForceDashLive";
														if (@QualityList !is null) AppendQualityList(QualityList, item, url);
														if (SelectBestProfile(final_itag, final_ext, itag, youtubeSets)) final_url = url;
														if (SelectBestProfile2(final_itag, final_ext, itag, youtubeSets)) final_url2 = url;
													}
								...
								if (captionTracks.isArray())
								{
									for(int j = 0, len = captionTracks.size(); j < len; j++)
									{
										...
										{											
											...
											if (baseUrl.isString())
											{
												string url = baseUrl.asString() + "&fmt=vtt";
...
bool PlaylistCheck(const string &in path)
{
	...
	if (url.find(YOUTUBE_USER_URL) >= 0 || url.find(YOUTUBE_CHANNEL_URL) >= 0) return true;
...
array<dictionary> PlaylistParse(const string &in path)
{
	...
	{	
		...
		string channelId = HostRegExpParse(url, "www.youtube.com(?:/channel|/user)/([-a-zA-Z0-9_]+)");
		if (!channelId.empty())
		{
			if (url.find(YOUTUBE_USER_URL) != -1)
			{
				url = "https://www.youtube.com/user/" + channelId;
				string dataStr = HostUrlGetString(RepleaceYouTubeUrl(url));

				channelId = GetEntry(dataStr, "content=\"https://www.youtube.com/channel/", "\"");
			}

			if (!channelId.empty() && channelId.substr(0, 2) == "UC")
			{
		...
		while (!dataStr.empty())
		{
			string jsonEntry = GetEntry(dataStr, "{\"responseContext\":", "};");
			...
			if (!jsonEntry.empty())
			{
				...
				jsonEntry = "{\"responseContext\":" + jsonEntry + "}";
				if (reader.parse(jsonEntry, root) && root.isObject())
				{
					...
					if (!lastvideoId.empty())
					{
						...
						dataStr = HostUrlGetString(RepleaceYouTubeUrl(url));
					}
					HostIncTimeOut(5000);
				}
			}
		...
		while (!dataStr.empty())
		{
			...
			{			
				...
				if (!moreUrl.empty())
				{
					...
					string json = HostUrlGetString(url, "", "x-youtube-client-name: 1\r\nx-youtube-client-version: 1.20200609.04.02\r\n");
					JsonReader Reader;
					JsonValue Root;				
