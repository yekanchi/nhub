...
; --- String Replace --- nsis.sourceforge.net/Another_String_Replace_(and_Slash/BackSlash_Converter)
!ifdef StrReplace
...
Function CheckDirExe
	IfFileExists "$EXEDIR\${APPDIR}\${APPEXE}" CheckDirEnd
!ifdef APPEXE64
	IfFileExists "$EXEDIR\${APPDIR64}\${APPEXE64}" CheckDirEnd
	MessageBox MB_OK|MB_ICONEXCLAMATION `${APPEXE64} was not found in $EXEDIR\${APPDIR64}`
	Abort
!endif
...
!ifdef APPEXE64
	...
	ReadINIStr $0 "$EXEDIR\${APP}Portable.ini" "${APP}Portable" "AllowMultipleInstances"
	StrCmp $0 "true" SecondLaunch64
		MessageBox MB_OK|MB_ICONINFORMATION `Another ${APPNAME} 64 bit is running.$\nClose ${APPNAME} before running ${APPNAME} Portable.`
		Abort
SecondLaunch64:
	SetOutPath "$EXEDIR\${APPDIR64}"
	${GetParameters} $0
	Exec `"$EXEDIR\${APPDIR64}\${APPEXE64}"${APPSWITCH} $0`
	Abort
!endif
Goto CheckRunEnd
SecondLaunch:
